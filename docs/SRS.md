...
| D26 | **A project's lead and its account manager see their own project's money** (Q21) — fee, money budget, retainer fee and billing amounts — as do `pjm:commercial` holders (C-level, entity directors, finance) over the entity. **Changing** an amount stays with `pjm:commercial`. Nobody else sees an amount, and nothing salary-derived is visible without `pjm:cost`. **Finance also holds `pjm:portfolio`** (owner, 2026-09-23), so it opens the project pages behind the invoices it raises; capacity planning and report/timesheet compliance stay with the people who run teams (`work:manage`), not with finance. |
| D27 | **Every client's work is accepted before it is billed** (Q22): a signed biên bản nghiệm thu per client-facing or billing milestone and per retainer month for **all clients**, not a chosen few. Close-out refuses an unaccepted client project unless the lead overrides with a recorded reason. The owner **accepted the seeded template's wording on 2026-09-23** ("ok with current template, may edit later"), so it carries no draft notice and HR edits it on Admin → Document templates; counsel reading it once is still worth the hour, since it is now the door every client invoice passes through. |
| D28 | **PJM replaces the Google Sheets** the teams run their work in, with a **cut-over on Monday 5 October 2026** (Q23; date chosen by the build, the owner may move it): from that morning the app is the record — the sheets stay readable for history and are no longer updated, and work that is not in the app does not count. The fortnight before it is for filling in the real teams, clients, retainers and open projects. |
