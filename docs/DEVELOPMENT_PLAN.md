...
> **Known gaps:** (1) a future-dated assignment or start date reaches `person` (what RBAC and sign-in read) only when something is next saved for that person — needs a daily roll-over job once the jobs framework (Phase 0 remainder) exists; the same job must turn `preboarding` into `active` on the start date. (2) FR-CHR-01 is partly covered: emergency contacts, education, certificates and photo are not modelled yet; the restricted fields come with encryption in week 2. (3) No duplicate-person warning on add, and no rehire screen (the schema allows a second employment period). (4) No screen for the employee-code scheme (defaults to `<ENTITY>-0001`). (5) The list has no export yet (FR-PLT-37). (6) Forms show one message per failure, not per-field errors; not yet clicked through in a browser or checked on a phone. (7) Re-pointing a work email hands that person's self-service view to the new address — audited, and blocked for role holders without `rbac:manage`, but there is no notification to the person yet. (8) Secondary assignments have a schema but no screens. (9) Page `<title>`s are hard-coded English, as in Phase 0.
> **Next (week 2):** contracts and alerts, dependents, document vault, encrypted restricted fields with read-auditing — these need file storage, notifications and the encryption key setup left over from Phase 0.
