...
import { listOwnAnomalies } from "@/modules/attendance/anomalies";
...
export default async function AttendancePage({ searchParams }: PageProps<"/attendance">) {
  ...
  const [personMonth, monthRow, anomalies, requests] = await Promise.all([getPersonMonth(personId, month), getMonthRow(personId, month), own ? listOwnAnomalies(personId, month) : [], own ? listAttendanceRequestsOf(personId, { from: monthStart(month), to: monthEnd(month) }) : []]);
