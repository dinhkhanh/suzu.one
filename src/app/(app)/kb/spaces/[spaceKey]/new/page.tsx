...
export default async function NewPagePage(props: PageProps<"/kb/spaces/[spaceKey]/new">) {
  ...
  return (
    <div className="flex max-w-3xl flex-col gap-6">
      ...
      <NewPageForm spaceId={loaded.space.id} parents={parents.map((node) => ({ id: node.id, title: node.title, depth: node.depth }))} defaultParentId={parents.some((node) => node.id === parentId) ? parentId : ""} />
    </div>
  );
}
