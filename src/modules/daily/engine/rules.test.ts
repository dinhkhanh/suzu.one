...
describe("merging several teams' rules", () => {
  it("no team: nothing is asked", () => {
    expect(mergeRules([])).toEqual(NO_TEAM_RULES);
    expect(reportRequirement(mergeRules([]), day("2026-09-21"))).toEqual({ required: false, reason: "optional" });
  });

  it("one team: its own rules", () => {
    expect(mergeRules([DEFAULT_TEAM_RULES])).toEqual({ planMode: "optional", reportMode: "required", reportDays: [1, 2, 3, 4, 5], planCutoff: "09:30", reportDeadline: "18:30", timeMode: "optional", timesheetApproval: false, coverMinDays: 2 });
  });
...
describe("is a report required today", () => {
  ...
  it("on an untracked Saturday (D15): no, unless the team lists Saturday", () => {
    expect(reportRequirement(rules, day("2026-09-26", { kind: "untracked" }))).toEqual({ required: false, reason: "untracked" });
    const saturdays = mergeRules([team({ reportDays: [1, 2, 3, 4, 5, 6] })]);
    expect(reportRequirement(saturdays, day("2026-09-26", { kind: "untracked" }))).toEqual({ required: true, reason: null });
  });
  it("on a rest day or a day the team does not report: no", () => {
    ...
    // Nobody scheduled the person: the weekday decides.
  ...
  it("the plan follows the plan mode and the report's days", () => {
    expect(planRequirement(rules, day("2026-09-21"))).toEqual({ required: false, reason: "optional" });
    const planning = mergeRules([team({ planMode: "required" })]);
    expect(planRequirement(planning, day("2026-09-21"))).toEqual({ required: true, reason: null });
    expect(planRequirement(planning, day("2026-09-26", { kind: "untracked" }))).toEqual({ required: false, reason: "untracked" });
    expect(planRequirement(planning, day("2026-09-21", { leave: "full" }))).toEqual({ required: false, reason: "leave" });
  });
  it("Today says 'day off' on holidays, leave, rest and unreported untracked days", () => {
    expect(isDayOff(rules, day("2026-09-26", { kind: "untracked" }))).toBe(true);
    expect(isDayOff(rules, day("2026-09-21", { leave: "full" }))).toBe(true);
    expect(isDayOff(rules, day("2026-09-21"))).toBe(false);
    expect(isDayOff(mergeRules([team({ reportDays: [1, 2, 3, 4, 5, 6] })]), day("2026-09-26", { kind: "untracked" }))).toBe(false);
  });
});

describe("lateness", () => {
  it("counts from the deadline of the report's own day, Vietnam time", () => {
    // 18:30 in Hanoi is 11:30 UTC.
    expect(isLate(new Date("2026-09-21T11:29:59Z"), "2026-09-21", "18:30")).toBe(false);
    expect(isLate(new Date("2026-09-21T11:30:00Z"), "2026-09-21", "18:30")).toBe(false);
