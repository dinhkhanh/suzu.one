...
import { canApproveTimesheet, canViewTimeEntry, canViewTimesheet, type ReportReader, type TimeReader } from "./policy";
import { rulesOfPeople } from "./team-rules";
import { listTimeOf, TIME_BACKFILL_DAYS, type TimeEntryView } from "./time";
...
export type RowLabel = { key: RowKey; taskId: string | null; category: string | null; taskKey: string | null; title: string | null; projectName: string | null };
...
const labelOf = (entry: TimeEntryView): RowLabel => ({ key: rowKeyOf(entry), taskId: entry.taskId, category: entry.category, taskKey: entry.key, title: entry.title, projectName: entry.projectName });

async function buildWeekView(subject: Subject, weekStart: IsoDate, today: IsoDate, options: { keep: (entry: TimeEntryView) => boolean; hints: boolean; own: boolean; canApprove: boolean; partial: boolean }): Promise<TimeWeekView> {
  ...
  const visible = all.filter(options.keep);
...
export async function getMyTimeWeek(personId: string, weekStart: IsoDate, today: IsoDate): Promise<TimeWeekView | null> {
  ...
  return buildWeekView(subject, weekStart, today, { keep: () => true, hints: true, own: true, canApprove: false, partial: false });
}
 ...
 * attendance hint is for the person and their approvers (FR-PJM-26).
 */
export async function getTimesheetView(reader: TimeReader, personId: string, weekStart: IsoDate, today: IsoDate): Promise<TimeWeekView | null> {
  ...
  const view = await buildWeekView(subject, weekStart, today, { keep: (entry) => canViewTimeEntry(reader, subject, entry), hints: canApprove, own: false, canApprove, partial: !full });
