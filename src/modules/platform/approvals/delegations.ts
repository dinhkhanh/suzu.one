 ...
 * Only active stand-ins who are not collaborators count, and never the requester or the person the
 * request is about (nobody approves their own request): the approver is then asked themselves.
 */
export async function standIns(executor: Executor, approverIds: readonly string[], context: { requestType: string; requesterId: string; subjectId?: string | null; day?: IsoDate }): Promise<Map<string, string>> {
  ...
  const rows = await executor
    ...
    .where(and(isNull(schema.approvalDelegation.revokedAt), lte(schema.approvalDelegation.validFrom, day), gte(schema.approvalDelegation.validTo, day), eq(schema.person.status, "active"), ne(schema.person.workforceType, "collaborator")))
  ...
  for (const approverId of approverIds) {
    const to = followDelegations(approverId, next, new Set(context.subjectId ? [context.requesterId, context.subjectId] : [context.requesterId]));
