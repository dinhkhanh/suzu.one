...
export type LeaderView = { people: { personId: string | null; name: string | null; tasks: LeaderTask[]; counts: Record<"todo" | "in_progress" | "in_review", number>; overdue: number; atRisk: number }[]; totals: { open: number; overdue: number; atRisk: number } };
...
export async function getLeaderView(viewer: WorkViewer, today: IsoDate): Promise<LeaderView> {
  ...
  if (!self) return { people: [], totals: { open: 0, overdue: 0, atRisk: 0 } };
  ...
  const items = await listItems(and(open, scope, or(isNull(schema.task.assigneePersonId), ne(schema.task.assigneePersonId, self))), db(), 1000);
  if (items.length === 0) return { people: [], totals: { open: 0, overdue: 0, atRisk: 0 } };
  ...
  const rank = (task: LeaderTask) => (task.risk === "overdue" ? 0 : task.risk === "at_risk" ? 1 : 2);
  ...
  people.sort((a, b) => Number(a.personId === null) - Number(b.personId === null) || b.overdue - a.overdue || b.atRisk - a.atRisk || (a.name ?? "").localeCompare(b.name ?? "", "vi"));
  return { people, totals: { open: tasks.length, overdue: tasks.filter((task) => task.risk === "overdue").length, atRisk: tasks.filter((task) => task.risk === "at_risk").length } };
}
...
export type MyWorkItem = { id: string; key: string; title: string; status: "todo" | "in_progress" | "done" | "cancelled"; stateName: string; dueDate: string | null; priority: number | null; projectName: string | null; reviewStatus: string; blockedBy: number };
...
export async function listMyWorkItems(personId: string): Promise<MyWorkItem[]> {
  const rows = await db()
    ...
    .where(and(eq(schema.task.kind, WORK_KIND), eq(schema.task.assigneePersonId, personId), open, isNull(schema.task.deletedAt)))
