...
beforeAll(async () => {
  ...
  for (const [key, name] of [["long", "Long Dang"], ["tam", "Tam Bui"], ["huy", "Huy Ho"], ["an", "An Le"], ["khoa", "Khoa Vu"], ["dir", "Dung Director"]] as const) {
    ...
    ids[key] = row.id;
    users[key] = { userId: `user-${key}`, sessionId: `session-${key}`, reauthAt: null, preferences: { locale: null, theme: null }, email: `${key}@suzu.group`, name, image: null, person: row, impersonator: null, principal: { personId: row.id, workforceType: "employee", grants: grants[key] ?? [] }, request: { ipAddress: null, userAgent: null } };
  }
  // Huy reports to Tam.
