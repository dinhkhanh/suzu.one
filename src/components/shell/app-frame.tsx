"use client";
import { Asterisk, ChevronDown, Menu, PanelLeft, Search, X } from "lucide-react";
...
import { readCollapsed, readCollapsedOnServer, subscribeCollapsed, writeCollapsed } from "@/components/shell/sidebar-store";
import { cn } from "@/lib/utils";

export type NavRow = { key: string; href?: string; label: string; count?: number };
...
function Row({ row, collapsed, active, soonLabel }: { row: NavRow; collapsed: boolean; active: boolean; soonLabel: string }) {
  const body = (
    <>
      <NavIcon name={row.key} />
      {collapsed ? null : <span className="min-w-0 flex-1 truncate">{row.label}</span>}
      {!collapsed && row.count ? <span className="nav-count">{row.count > 99 ? "99+" : row.count}</span> : null}
    </>
  );
 ...
 */
export function AppFrame({ labels, pinned, main, admin, user, footer, children }: Props) {
  ...
  const group = (label: string, items: NavRow[]) =>
    items.length === 0 ? null : (
      <div className="flex flex-col gap-0.5">
        {collapsed ? <div className="mx-2 my-2 border-t" /> : <p className="nav-section">{label}</p>}
        {items.map((row) => (
          <Row key={row.key} row={row} collapsed={collapsed} active={row.key === activeKey} soonLabel={labels.soon} />
        ))}
      </div>
    );
  ...
  return (
    <div className="flex h-dvh flex-col bg-canvas p-0 md:p-2.5">
      <div className="shell-card">
        ...
        {open ? <button type="button" aria-label={labels.close} className="fixed inset-0 z-30 bg-black/30 md:hidden" onClick={() => setOpen(false)} /> : null}
        ...
        <aside
          ...
          className={cn(
            "fixed inset-y-0 left-0 z-40 flex w-[264px] shrink-0 flex-col border-r border-border bg-sidebar transition-transform md:static md:z-auto md:translate-x-0 md:transition-[width]",
            collapsed && "md:w-16",
            open ? "translate-x-0" : "-translate-x-full",
          )}
        >
          <div className={cn("flex h-14 shrink-0 items-center gap-2 px-3", collapsed && "md:justify-center md:px-0")}>
            <Link href="/home" className="flex min-w-0 items-center gap-2">
              ...
              {collapsed ? null : (
                <>
                  <span className="truncate text-[0.9375rem] font-semibold tracking-[-0.015em]">{labels.workspace}</span>
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label={labels.close} className="ml-auto flex size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted md:hidden">
            ...
            >
              ...
              {collapsed ? null : (
                <>
                  ...
                  <kbd className="rounded border border-border px-1 font-sans text-[0.6875rem] text-faint">⌘K</kbd>
                </>
              )}
            </button>
          </div>

          <nav className={cn("flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto px-3 pt-2 pb-3", collapsed && "md:px-2")}>
            {pinned.map((row) => (
              <Row key={row.key} row={row} collapsed={collapsed} active={row.key === activeKey} soonLabel={labels.soon} />
            ))}
          ...
          <div className={cn("flex shrink-0 flex-col gap-2 border-t border-border p-3", collapsed && "md:items-center md:px-2")}>
            <div className="flex items-center gap-2">
              <span className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-muted text-[0.6875rem] font-semibold text-muted-foreground">{initials}</span>
              {collapsed ? null : (
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border px-4">
            <button type="button" onClick={() => setOpen(true)} aria-label={labels.menu} className="flex size-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted md:hidden">
              ...
              {crumbs.map((crumb, index) => {
                ...
                return (
                  <span key={crumb.key} className="flex min-w-0 items-center gap-2">
                    ...
                    <Link href={crumb.href ?? "/home"} className={cn("truncate", last ? "font-medium text-foreground" : "text-muted-foreground hover:text-foreground")}>
                      {crumb.label}
                    </Link>
                  </span>
                );
            ...
            {crumbs.at(-1)?.count ? (
              <Badge variant="secondary" className="shrink-0">
                {(crumbs.at(-1)?.count ?? 0) > 99 ? "99+" : crumbs.at(-1)?.count}
              </Badge>
            ) : null}
          </header>

          <main className="min-h-0 min-w-0 flex-1 overflow-y-auto px-4 py-5 md:px-6">{children}</main>
        </div>
      </div>
    </div>
  );
