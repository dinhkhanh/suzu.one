...
- A page asks the cache first and Postgres only for what the cache does not hold: reads go through `cached()` / `invalidate()` from `src/lib/cache` (Upstash Redis) in three tiers with their own TTL (`TTL.reference`, `TTL.personal`, `TTL.live`). *Reference* data (org tree, types, policies, rates, calendars, templates, role grants): one key for the whole small table, long TTL. *Personal* data (a person's session, row, profile, memberships, saved views): one key per person, short TTL. *Live* data (badges, inboxes, the Today page, the notification list): one key per person and screen via `src/lib/cache/live.ts`, the shortest TTL, dropped for the actor after every action (`createAction`) and for the recipients of every `notify()`. Restricted and compensation data are never cached. Every writer of a cached table calls `invalidate()` after its change commits — or, when it only has the caller's transaction, as it writes: the 30-second stale marker keeps readers off the cache until the commit lands, so a transaction that holds a cached table must commit well within that. A reader given a transaction reads from it, not the cache. Writes made outside the app (seeds, manual SQL) are followed by `pnpm cache:flush`.
- Postgres functions live in the private `app` schema (never `public`, which Supabase's API exposes), are `LANGUAGE sql STABLE` where possible, pin their path (`SET search_path = ''` with schema-qualified names; trigger functions in `public` may use `public, pg_temp`), and end with `REVOKE EXECUTE ... FROM PUBLIC`. A migration test enforces this. Prefer aggregate SQL (GROUP BY, FILTER, DISTINCT ON, LATERAL) to loading rows into JS.
