-- Supabase grants its public API roles access to everything in `public` by default. This app never
-- uses the Supabase data API, so take those privileges away (row-level security is the second lock).
-- The roles do not exist on plain Postgres (tests, other hosts), hence the guard.
DO $$
DECLARE
  api_role text;
BEGIN
